...
priorityQueue initialize(int maxElements){
    if(pthread_rwlock_init(&rwlock, NULL)){
        printf("Read/Write lock initialize failed!");
        exit(1);
    }
...
HelloWorldData_Msg* deleteMin(priorityQueue pQ){
    int i, child;
    elementStruct minElement, lastElement;
    if(isEmpty(pQ)){
        ...
        while(index <= pQ->size){
            if(time(NULL) - pQ->ele[index].startTime >= thresholdTime){
                ...
                if(pQ->ele[index].priority - 1 >= 0){
                    pthread_rwlock_wrlock(&rwlock);
                    pQ->ele[index].priority -= 1;
                    pthread_rwlock_unlock(&rwlock);
                    printf("Priority of Message:%s has been raised to %d\n", pQ->ele[index].msg->message, pQ->ele[index].priority);
